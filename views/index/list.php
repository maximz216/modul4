<?php foreach ($data as $category => $item) {
    ...
    <?php foreach ($b as $key => $item): ?>
        <h2><a href="/category/show/<?= $item ?>"><?= $key ?></a></h2>
        <?php foreach ($data as $category): ?>
            <?php if (isset($category['id_category']) && $category['id_category'] == $item): ?>
                <ul class="show-unstyled">
                    <li><a href="/news/show/<?= $category['id_news'] ?>"><?= $category['title_news']; ?></a></li>
                </ul>
            <?php endif; ?>
        <?php endforeach; ?>
    <?php endforeach; ?>
<div class="slider">
    <ul>
        ...
        <?php if ($cnt == 5) break; ?>
        <li>
            <a href="/news/show/<?= $value['id_news']; ?>">
...
<table class="table table-striped"  width="400px">
    ...
    <?php $i=1;foreach ($data['themes'] as $theme): ?>
        <tr>
            <td><?=$i++?></td>
            <td><a href="/news/show/<?=$theme['id_news']?>"><?=$theme['title_news']?></a></td>
            <td><?=$theme['datet']?></td>
        </tr>
    <?php endforeach; ?>
    </tbody>
