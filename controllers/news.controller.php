...
class NewsController extends Controller
{
    ...
    public function show()
    {
        ...
        $this->data['news'] = $this->model->getNewsshowByPage($page, 10);
        if (isset($params)&& !isset($_GET['pages'])) {
            $id = $params[0];
            $this->data = $this->model->getNewsshowById($id);
...
//            if(isset($_POST['submit'])&& isset($_POST['comment']) && !empty($_POST['comment'])){
    ...
    public function tag()
    {
        ...
        if (isset($params)) {
            $id = $params[0];
            $this->data['tags'] = $this->model->getNewsshowByTagId($id);
        }else{
    ...
    public function admin_edit()
    {
        if ($_POST) {
            ...
            Router::redirect('/admin/index/show');
        }

        if (isset($this->params[0])) {
            $this->data = $this->model->getNewsshowById($this->params[0]);
            $this->data['category'] = $this->model->getCategoryshow();
            $this->data['tags_show'] = $this->model->getTagsshow();
    ...
    public function admin_show()
    {
        ...
        $page = !isset($page) ? 0 : $page;
        $this->data = $this->model->getNewsshowByPage($page, 10);
    }
